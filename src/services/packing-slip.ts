...
export default class PackingSlipService extends TransactionBaseService {
  ...
  private calculateTemplateKind(documentSettings: DocumentSettings, documentInvoiceSettings: DocumentInvoiceSettings) : PackingSlipTemplateKind {
    // if (documentInvoiceSettings && documentInvoiceSettings.invoice_template) {
    //   return documentInvoiceSettings.invoice_template as PackingSlipTemplateKind;
    // }
    // // Legacy
  ...
  async generatePreview(templateKind: PackingSlipTemplateKind) : Promise<PackingSlipResult> {
    ...
    if (lastOrder.length > 0) {
      ...
      if (settings) {
        ...
        previewPackingSlip.number = '1';
        previewPackingSlip.display_number = '111000';
